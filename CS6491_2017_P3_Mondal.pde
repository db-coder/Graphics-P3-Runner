...
float 
  ...
  s=0;
int
  f=0, maxf=2*30, level=6, method=4;
String SDA = "angle";
float defectAngle=0;
...
void setup() {
  myFace = loadImage("data/Dibyendu.jpg");  // load image from file pic.jpg in folder data *** replace that file with your pic of your own face
  textureMode(NORMAL);          
...
void draw() {
 ...
 ////     animating=true;  
  ...
  if(filming && (animating || change)) saveFrame("FRAMES/F"+nf(frameCounter++,4)+".jpg");  // save next frame to make a movie
